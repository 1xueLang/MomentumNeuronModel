...
###### learnable_lb, mt
class bMomentumNeuronFunc0(torch.autograd.Function):
    ...
    def forward(ctx, x, tau, th, momentum, lamb, suro, alpha):
        ...
        with cp.cuda.Device(x.get_device()):
            bMomentumNeuronFunc0.neuron_FP((cuda_blocks(ctx.N * ctx.D),), (cuda_threads,), (
    ...
    def backward(ctx, gradout):
        ...
        with cp.cuda.Device(gradout.get_device()):
            bMomentumNeuronFunc0.neuron_BP((cuda_blocks(ctx.N * ctx.D),), (cuda_threads,), (
...
###### learnable_lb
class bMomentumNeuronFunc1(torch.autograd.Function):
    ...
    def forward(ctx, x, tau, th, momentum, lamb, suro, alpha):
        ...
        with cp.cuda.Device(x.get_device()):
            bMomentumNeuronFunc1.neuron_FP((cuda_blocks(ctx.N * ctx.D),), (cuda_threads,), (
    ...
    def backward(ctx, gradout):
        ...
        with cp.cuda.Device(gradout.get_device()):
            bMomentumNeuronFunc1.neuron_BP((cuda_blocks(ctx.N * ctx.D),), (cuda_threads,), (
...
###### learnable_mt
class bMomentumNeuronFunc2(torch.autograd.Function):
    ...
    def forward(ctx, x, tau, th, momentum, lamb, suro, alpha):
        ...
        out1, out2, out3, out4 = [torch.zeros_like(x) for i in range(4)]
        with cp.cuda.Device(x.get_device()):
            bMomentumNeuronFunc2.neuron_FP((cuda_blocks(ctx.N * ctx.D),), (cuda_threads,), (
                ten2cpy(x), ten2cpy(out1), ten2cpy(out2), ten2cpy(out3), ten2cpy(out4),
                cp.float32(tau[0].item()), cp.float32(th[0].item()), cp.float32(momentum[0].item()), cp.float32(lamb[0].item()), 
                cp.int32(ctx.N), cp.int32(ctx.T), cp.int32(ctx.D)
            ))
    ...
    def backward(ctx, gradout):
        ...
        gradin2 = torch.zeros_like(gradout[:, 0])
        with cp.cuda.Device(gradout.get_device()):
            bMomentumNeuronFunc2.neuron_BP((cuda_blocks(ctx.N * ctx.D),), (cuda_threads,), (
...
###### unlearnable
class bMomentumNeuronFunc3(torch.autograd.Function):
    ...
    def forward(ctx, x, tau, th, momentum, lamb, suro, alpha):
        ...
        out1, out2, out3 = [torch.zeros_like(x) for i in range(3)]
        with cp.cuda.Device(x.get_device()):
            bMomentumNeuronFunc3.neuron_FP((cuda_blocks(ctx.N * ctx.D),), (cuda_threads,), (
                ten2cpy(x), ten2cpy(out1), ten2cpy(out2), ten2cpy(out3),
                cp.float32(tau[0].item()), cp.float32(th[0].item()), cp.float32(momentum[0].item()), cp.float32(lamb[0].item()), 
                cp.int32(ctx.N), cp.int32(ctx.T), cp.int32(ctx.D)
            ))
    ...
    def backward(ctx, gradout):
        ...
        gradin1 = torch.zeros_like(gradout)
        with cp.cuda.Device(gradout.get_device()):
            bMomentumNeuronFunc3.neuron_BP((cuda_blocks(ctx.N * ctx.D),), (cuda_threads,), (
...
class bMomentumLIF(nn.Module):
    r""" STBP based balancd momentum neuron model(bMomentumLIF).
    ...
    def analysis_momentum_func(self):
        MNFunc = {
            0: bMomentumNeuronFunc0,
            1: bMomentumNeuronFunc1,
            2: bMomentumNeuronFunc2,
            3: bMomentumNeuronFunc3,
        }
